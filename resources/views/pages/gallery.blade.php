...
@section('meta_description', 'Browse our complete gallery of handcrafted photo frames, 3D shadow boxes, and quilling art.')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            @foreach($products as $product)
                <a href="{{ asset($product['image']) }}" class="glightbox block group rounded-2xl overflow-hidden border border-[var(--color-border)] aspect-square bg-white shadow-xs">
                    <img src="{{ asset($product['image']) }}" alt="{{ $product['title'] }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                </a>
            @endforeach
        </div>
